...
export function registerCreateUnitTool(
  ...
  server.registerTool(
    'create_unit',
    {
      ...
      description:
        'Preview or create a Federation body, vehicle template and specification document. Writing requires confirm=true.',
      inputSchema: z.object({
        repositoryRoot: z.string().optional(),
        id: z.string().regex(/^FED-H\d{2,3}$/),
        ...
        hitpoints: z.number().int().positive().default(720),
        armourKinetic: z.number().int().nonnegative().default(45),
        armourHeat: z.number().int().nonnegative().default(55),
        buildPower: z.number().int().positive().default(190),
        buildPoints: z.number().int().positive().default(420),
        weight: z.number().int().positive().default(5800),
        energyCapacity: z.number().positive().default(280),
        heatCapacity: z.number().positive().default(100),
        shieldCapacity: z.number().nonnegative().default(180),
        placeholderModel: z.string().default('Viper.PIE'),
    ...
    async (input) => {
      ...
      const specDirectory = path.join(root, 'docs/units/federation');
      const specPath = path.join(specDirectory, `${input.id}.md`);

      const bodies = await readJsonObject(bodyPath);
      const templates = await readJsonObject(templatePath);

      const duplicateIds = [
        input.id in bodies ? input.id : null,
        input.templateId in templates ? input.templateId : null,
      ].filter((value): value is string => value !== null);

      if (duplicateIds.length > 0) {
        throw new Error(`Refusing to overwrite existing IDs: ${duplicateIds.join(', ')}`);
      }
      ...
      bodies[input.id] = body;
      templates[input.templateId] = template;

      await writeFile(bodyPath, `${JSON.stringify(sortRecord(bodies), null, 2)}\n`, 'utf8');
      await writeFile(templatePath, `${JSON.stringify(sortRecord(templates), null, 2)}\n`, 'utf8');
