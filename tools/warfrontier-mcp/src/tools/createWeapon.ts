...
export function registerCreateWeaponTool(
  server: McpServer,
  ...
  server.registerTool(
    'create_weapon',
    {
      ...
      description:
        'Preview or create a Federation weapon prototype and specification document. Writing requires confirm=true.',
      inputSchema: z.object({
        repositoryRoot: z.string().optional(),
        id: z.string().regex(/^FED-WPN-\d{3}$/),
        name: z.string().min(3).max(100),
        role: z.string().min(3).max(100),
        damage: z.number().int().positive().default(34),
        firePause: z.number().int().positive().default(5),
        shortRange: z.number().int().positive().default(768),
        longRange: z.number().int().positive().default(1536),
        shortHit: z.number().int().min(0).max(100).default(98),
        longHit: z.number().int().min(0).max(100).default(98),
        energyPerSecond: z.number().nonnegative().default(4),
        heatPerSecond: z.number().nonnegative().default(8),
        maximumDurationSeconds: z.number().positive().default(2.5),
        weaponClass: z.enum(['HEAT', 'KINETIC']).default('HEAT'),
        weaponEffect: z.string().default('ANTI PERSONNEL'),
        weaponSubClass: z.string().default('LAS_SAT'),
        placeholderModel: z.string().default('GNMLASER.PIE'),
        placeholderMountModel: z.string().default('TRMLASER.PIE'),
        placeholderMuzzleGfx: z.string().default('FXLASER.PIE'),
        placeholderHitGfx: z.string().default('FXLASER.PIE'),
        placeholderSound: z.string().default('lasstrik.ogg'),
        buildPower: z.number().int().positive().default(140),
        buildPoints: z.number().int().positive().default(360),
        weight: z.number().int().positive().default(2600),
    ...
    async (input) => {
      ...
      const specDirectory = path.join(root, 'docs/weapons/federation');
      const specPath = path.join(specDirectory, `${input.id}.md`);
      const weapons = await readJsonObject(weaponPath);

      if (input.id in weapons) {
        throw new Error(`Refusing to overwrite existing weapon ID: ${input.id}`);
      }
      ...
      const weapon = {
        ...
        flightSpeed: 2500,
        hitGfx: input.placeholderHitGfx,
        hitpoints: 180,
        id: input.id,
        lightWorld: 1,
        longHit: input.longHit,
        longRange: input.longRange,
        maxElevation: 75,
        minElevation: -20,
        minimumDamage: 35,
        missGfx: input.placeholderHitGfx,
        model: input.placeholderModel,
        mountModel: input.placeholderMountModel,
        movement: 'DIRECT',
        muzzleGfx: input.placeholderMuzzleGfx,
        name: input.name,
        numExplosions: 1,
        recoilValue: 10,
        reloadTime: 0,
        rotate: 180,
        shortHit: input.shortHit,
        shortRange: input.shortRange,
        waterGfx: 'FXSSplsh.PIE',
        weaponClass: input.weaponClass,
        weaponEffect: input.weaponEffect,
        weaponSubClass: input.weaponSubClass,
        weaponWav: input.placeholderSound,
        weight: input.weight,
        warfrontier: {
      ...
      await writeFile(weaponPath, `${JSON.stringify(sortRecord(weapons), null, 2)}\n`, 'utf8');
      await writeFile(specPath, specification, { encoding: 'utf8', flag: 'wx' });

      return textResult({
        ...preview,
        written: [path.relative(root, weaponPath), path.relative(root, specPath)],
